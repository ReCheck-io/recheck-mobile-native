...
const chainClient = {
  setInstance(newInstance = 'ReCheckAPP') {
    e2e.setDefaultRequestId(newInstance)
  },

  setURLandNetwork(apiURL, apiNetwork) {
    if (apiURL !== '') {
      apiUrl = apiURL;
    }
    if (apiNetwork !== '') {
  ...
  saveWallet(keyPair, password) {
    ...
    let encrypted = encrypt(password, keyPairStr);
    logger('encrypted private key', encrypted);
    // web3.eth.accounts.wallet.accounts[0].privateKey // encrypt wallet
    localStorage.walletAe1 = encrypted;
  ...
  loadWallet(password) {
    ...
    let account = decrypt(password, encrypted); // decrypt encrypted
    this.resetWallet();
  ...
  async restoreIdentityAtStart(password, phrase) {
    this.resetWallet()
  ...
  async importPrivateKey(password, phrase) {
    if (!this.checkPassword(password)) {
      return 'authError'
    }
  ...
  async doLogin(password, _challenge, callback) {
    ...
    try {
      ...
      let token = await e2e.loginWithChallenge(
        challenge, keyPair, firebaseToken, deviceInfo
      );
  ...
  async doExecSelection(password, _selection, callback) {
